...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (function(){
    ...
    try{
      ...
      if(stored){ showImage(stored); }
      else {
        // derive initials from stored name
        const n = localStorage.getItem('profileName') || document.getElementById('name-text')?.innerText || 'Châu Thế Phong';
        if(avatarInitials) avatarInitials.innerText = computeInitials(n);
      }
    }catch(e){ /* ignore */ }
    ...
    if(avatarInput){
      avatarInput.addEventListener('change', (e)=>{
        ...
        reader.onload = function(ev){
          ...
          try{ localStorage.setItem('profileAvatar', data); } catch(err){ console.warn('Cannot store avatar', err); }
          showImage(data);
        };
    ...
    const nameInputObserver = new MutationObserver(()=>{
      const n = document.getElementById('name-text')?.innerText || '';
      if(!localStorage.getItem('profileAvatar') && avatarInitials) avatarInitials.innerText = computeInitials(n);
    });
    const nameNode = document.getElementById('name-text');
    if(nameNode) nameInputObserver.observe(nameNode, { childList: true, characterData: true, subtree:true });
  })();
